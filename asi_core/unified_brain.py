...
from .hive_mind import HiveMind, ShareRecord
from .neural_dsl import (
    parse_program, to_source as ndl_to_source, netsearch, NeuronDefinition,
)
...
class UnifiedBrain:
    ...
    def __init__(
        ...
        # Hive mind: lazily initialized via init_hive_mind() -- most brains
        # never share extensions with another agent, so there's no reason
        # to construct a HiveMind (and its TrustSystem) for every instance.
    ...
    def define_neuron_from_ndl(self, source: str) -> Dict[str, NeuronDefinition]:
        ...
        for req in program.netsearch_requests:
            if req.net:
                netsearch(req.net)  # Could create neurons from results here
