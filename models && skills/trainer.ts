...
const MAX_ACCUMULATED_CORPUS_CHARS = 200_000;
...
export class NeuroclawTrainer {
   ...
   */
  private trainingLock: Promise<void> = Promise.resolve();
   ...
   */
  async learnText(text: string): Promise<void> {
  ...
  async train(text?: string): Promise<void> {
    const run = async () => {
      ...
      if (this.config.useElasticCore) await this.trainElasticCoreLayer(corpus);
      else await this.trainHiddenLayer(corpus);
  ...
  private async buildNGramTables(text: string): Promise<void> {
    ...
    for (let order = 1; order <= this.config.ngramOrder; order++) {
      ...
      for (let i = 0; i < text.length - order; i++) {
        if (i > 0 && i % YIELD_EVERY_CHARS === 0) await yieldToEventLoop();
  ...
  private async trainEmbeddings(text: string): Promise<void> {
  ...
  private async trainHiddenLayer(text: string): Promise<void> {
    const dim = this.config.hiddenDim;
    ...
    for (let epoch = 0; epoch < this.config.epochs; epoch++) {
      ...
      for (let i = ctx; i < text.length - 1; i++) {
        if (i > ctx && i % YIELD_EVERY_CHARS === 0) await yieldToEventLoop();
  ...
  private async trainElasticCoreLayer(text: string): Promise<void> {
    const dim = this.config.hiddenDim;
    const ctx = this.config.contextWindow;
