...
import { NeuroLangInterpreter } from "./core/neuro-lang.js";
...
export class NeuroclawLLM {
  ...
  private trainer: NeuroclawTrainer;
  private quantizer: BackgroundQuantizer;
  private valueAllocator: ValueRangeAllocator;
  private moeRouter: MoERouter;
  private mesh: NeuronMesh;
  private hyperEngine: HyperDimensionalEngine;
  private rlmTrainer: RLMTrainer;
  private thornsEngine: ThornsEngine;
  private projectId: string = "";
  private built = false;
  private trained = false;
  private context: string = "";
  private selfExtensions: Map<string, string> = new Map();
  private selfExtensionsDir: string;
  private generationCount = 0;
  private pipeline: NeuroPipeline | null = null;
  ...
  constructor(config: Partial<LLMConfig> = {}) {
    ...
    this.trainer = new NeuroclawTrainer(
      this.tokenizer.getVocabSize(),
      this.tokenizer.getCharToId(),
      this.tokenizer.getIdToChar(),
    ...
    this.thornsEngine.connectCore(
      this.valueAllocator, this.mesh, this.hyperEngine,
      this.rlmTrainer, this.moeRouter
    );
  ...
  build(): void {
    ...
    const outputNeurons: string[] = [];
    for (let i = 0; i < vocabSize; i++) {
      const n = this.builder.addNeuron(this.projectId, `vocab_${i}`, 2, { x: i * 20, y: 400 });
      if (n) outputNeurons.push(n.id);
    }
    if (outputLayer) outputLayer.neurons = outputNeurons;
    ...
    for (let i = 0; i < inputNeurons.length; i++) {
      for (let j = 0; j < hiddenNeurons.length; j++) {
        if (Math.random() < 0.3) {
          ...
          const from = inputNeurons[i]; const to = hiddenNeurons[j];
          if (from && to) this.builder.connectNeurons(this.projectId, from, to, w);
        }
      }
    }
    for (let i = 0; i < hiddenNeurons.length; i++) {
      for (let j = 0; j < outputNeurons.length; j++) {
        if (Math.random() < 0.3) {
          ...
          const from = hiddenNeurons[i]; const to = outputNeurons[j];
          if (from && to) this.builder.connectNeurons(this.projectId, from, to, w);
        }
      }
    }

    for (const nid of inputNeurons) this.builder.dragLabel(this.projectId, nid, "embedding");
    for (const nid of hiddenNeurons) this.builder.dragLabel(this.projectId, nid, "expert-hidden");
    for (const nid of outputNeurons) this.builder.dragLabel(this.projectId, nid, "vocab-logit");
    ...
    const neuronStates: NeuronState[] = allNeuronIds.map((id) => ({
  ...
  async generate(prompt: string, options: Partial<GenerateOptions> = {}): Promise<string> {
    ...
    for (let i = 0; i < this.config.embeddingDim; i++) embedding[i] = rawEmb[i] ?? 0;
    ...
    if (this.generationCount % 5 === 0) await this.createSelfExtension(prompt, finalOutput);
  ...
  private buildTextResponse(
    prompt: string,
    thorns: { response: string; intent: { intent: string; confidence: number }; crossCheck: { overallConfidence: number }; actionPlan: string[]; simulation: { bestAction: string } },
    hyper: { noveltyScore: number },
    rlm: { action: number; thinkingSteps: number[] },
    moe: { expertContributions: Map<string, number>; output: Float32Array },
  ): string {
    ...
    for (const word of keywords.slice(0, 4)) {
      ...
      if (t) {
        if (t.Y) contextParts.push(`${word}: ${t.Y.slice(0, 60)}`);
        else if (t.X.length > 0) contextParts.push(`${word} ≈ ${t.X.slice(0, 2).join(', ')}`);
      }
    }
    ...
    switch (intent) {
      ...
      case 'query': {
        ...
        for (const w of words.slice(0, 5)) {
          ...
          if (t?.Y) answerParts.push(`${w} means: ${t.Y}`);
          else if (t && t.X.length > 0) answerParts.push(`${w} (also: ${t.X.slice(0, 3).join(', ')})`);
        }
  ...
  private async createSelfExtension(prompt: string, output: string): Promise<void> {
    const extId = `self_ext_${this.generationCount}`;
    const extProject = this.builder.createProject(
      `Memory: ${prompt.slice(0, 30)}`,
      "Self-authored extension storing learned patterns"
    );
    ...
    if (saved) {
      ...
      if (quantized) writeFileSync(join(extDir, "model.q4.json"), quantized, "utf-8");
      appendFileSync(join(this.selfExtensionsDir, "index.jsonl"),
        JSON.stringify({ id: extId, prompt: prompt.slice(0, 100), time: Date.now() }) + "\n", "utf-8");
    }
  ...
  loadModel(model: { id?: string; name?: string; config?: Record<string, unknown>; weights?: Record<string, number[]> }): void {
    if (model.config) {
      const cfg = model.config as Partial<LLMConfig>;
      if (cfg.embeddingDim) this.config.embeddingDim = cfg.embeddingDim;
      if (cfg.hiddenDim) this.config.hiddenDim = cfg.hiddenDim;
      if (cfg.numExperts) this.config.numExperts = cfg.numExperts;
      if (cfg.contextLength) this.config.contextLength = cfg.contextLength;
    }
  ...
  getActiveModel(): { id: string; neurons: number; experts: number } | null {
  ...
  reloadSelfExtensions(): void {
    ...
    for (const line of lines) {
      try {
        const meta = JSON.parse(line) as { id: string; prompt: string; time: number };
    ...
    if (loaded > 0) console.error(`[NeuroClaw] Reloaded ${loaded} self-extension(s)`);
  }
  ...
  searchNeurons(query: string) { return this.builder.searchNeurons(this.projectId, query); }
  netSearch(query: string) { return this.builder.netSearch(this.projectId, query); }
  typeOutput(neuronId: string, inputValue: number) { return this.builder.typeModelOutput(this.projectId, neuronId, inputValue); }
  ...
  getBuilder(): ExtensionBuilder { return this.builder; }
  getTokenizer(): Tokenizer { return this.tokenizer; }
  getTrainer(): NeuroclawTrainer { return this.trainer; }
  getMoERouter(): MoERouter { return this.moeRouter; }
  isBuilt(): boolean { return this.built; }
  getPipeline(): NeuroPipeline | null { return this.pipeline; }

  private sampleFromProbs(probs: Float32Array, temperature: number): number {
    ...
    for (let i = 0; i < logits.length; i++) { scaled[i] = Math.exp(logits[i]! - maxLogit); sumExp += scaled[i]!; }
    if (sumExp <= 0) return 4;
    for (let i = 0; i < scaled.length; i++) scaled[i] = scaled[i]! / sumExp;
    let r = Math.random();
    for (let i = 0; i < scaled.length; i++) { r -= scaled[i]!; if (r <= 0) return i; }
