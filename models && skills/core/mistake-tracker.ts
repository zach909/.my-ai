...
export interface MistakeInput {
  ...
  assumption?: string;
  failedStep?: string;
  /** The specific skill/agent responsible, when cause is "incorrect-skill" (§5: "which skills are missing/incomplete" needs to name one, not just tally a count). */
  failedSkill?: string;
  prevention?: string;
}
...
export class MistakeTracker {
  ...
  record(input: MistakeInput): Mistake {
    ...
    if (existing) {
      ...
      if (input.failedSkill) existing.failedSkill = input.failedSkill;
      return existing;
    }
  ...
  skillBreakdown(): Record<string, number> {
    ...
    for (const m of this.mistakes.values()) {
      if (!m.failedSkill || m.resolved) continue;
