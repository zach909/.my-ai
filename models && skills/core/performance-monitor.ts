...
export class PerformanceMonitor {
  ...
  record(metricName: string, value: number, unit: string = ''): void {
    const now = Date.now();
  ...
  /** Get metric history */
  getMetricHistory(metricName: string, durationMs?: number): MetricPoint[] {
    const series = this.metrics.get(metricName);
    if (!series) return [];
    
    if (!durationMs) return [...series.points];
  ...
  getMetricStats(metricName: string, durationMs?: number): {
    min: number;
    max: number;
    avg: number;
    p50: number;
  ...
  private checkAnomalies(metrics: ComponentMetrics): void {
    const now = Date.now();
  ...
  /** Clear old anomalies */
  clearOldAnomalies(maxAgeHours = 24): void {
    const cutoff = Date.now() - (maxAgeHours * 60 * 60 * 1000);
    this.anomalies = this.anomalies.filter(a => a.timestamp >= cutoff);
  }

  /** Record a prediction and its actual outcome */
  recordPrediction(predictionName: string, predicted: number, actual: number): void {
  ...
  getPredictionAccuracy(predictionName?: string): {
    avgAccuracy: number;
    avgError: number;
    count: number;
  } | null {
