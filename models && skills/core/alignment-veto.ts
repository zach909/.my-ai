...
// rules, with one safety property from the notes:
//
//   Human in the loop for irreversible actions — anything irreversible or
//   externally-effecting requires confirmation, never silent auto-approval.
//
// (This module used to also fail safe on drift — the hyperdimensional
// engine's self-model surprise signal, section 10 -- but that self-model is
// gone; see onebrain.ts. There is no signal left to gate on here.)
//
...
/**
 * Reserved for future veto context. Carries nothing today -- it used to
 * carry the hyperdimensional engine's self-model surprise (drift) signal,
 * removed along with that self-model (see onebrain.ts).
 */
 ...
 */
function defaultScorer(action: ProposedAction, _ctx: AlignmentContext): number {
  let score = 1.0;
...
export class AlignmentVeto {
  ...
  evaluate(action: ProposedAction, ctx: AlignmentContext = {}): VetoDecision {
    ...
    // Rule 2: human in the loop for irreversible / external-effect actions.
    ...
    // Rule 3: low benevolence score → block.
