...
import { packZip, ZIP_FOLDERS, type ZipTree } from "./zip-halt.js";

// ─── The plugin side, seen structurally ─────────────────────────────────

/*
 * Structural rather than an import of plugin_manager/sdk.ts's ToolPlugin, so
 * the core does not depend on the plugin layer (the dependency already runs
 * the other way: the registry imports from core). A ToolPlugin satisfies this
 * as-is.
 */
...
export interface NeuronToolSource {
  getPluginId(): string;
  getTools(): ToolSpec[];
  callTool(name: string, args: Record<string, unknown>, origin: ToolOrigin): Promise<ToolCallEvent>;
  onToolCall(observer: (event: ToolCallEvent) => void): () => void;
}

/** What the Access page enforces. AccessManager satisfies this. */
...
/** A plugin's own input doorway: the pair of neurons its results arrive on. */
...
export interface AttachResult {
  plugin: string;
  /** tool name -> its output neuron */
  tools: Record<string, number>;
  channel: ResultChannel | null;
  /** Neurons this attach added to the network (0 when it reused ones already there). */
  added: number;
  skipped?: string;
}
...
export interface ToolFiring {
  ...
  neuronId: number;
  energy: number;
  /** The line it had to clear on the tick it fired. */
  line: number;
}

/** One tool's answer in a decision. */
export interface ToolOption {
  ...
  neuronId: number;
  energy: number;
  /** 0..1; above 0.5 exactly when the neuron fires. */
  score: number;
  fires: boolean;
}

/** Every tool's answer at once, highest score first. */
export interface ToolDecision {
  /** The firing line these scores were measured against. */
  line: number;
  options: ToolOption[];
  /** The tools that fire -- none, one, or several. */
  chosen: string[];
}

/** The group every tool neuron of one plugin carries: the plugin's output layer. */
export const toolOutputGroup = (plugin: string): string => `tool:${plugin}`;
/** The group a plugin's result-channel neurons carry: the plugin's input layer. */
export const toolInputGroup = (plugin: string): string => `tool-result:${plugin}`;

/** Where, in the Zip Loop's output archive, the network writes a call's arguments. */
export const toolCallPath = (plugin: string, tool: string): string => `${ZIP_FOLDERS.plugins}${plugin}/${tool}.json`;
/** Where, in the archive fed back in, a successful call's result sits. */
export const toolResultPath = (plugin: string, tool: string): string => `${ZIP_FOLDERS.plugins}${plugin}/${tool}.result.json`;
/** Where a failed call's error sits -- the plugins/error/ folder zip-halt.ts already names for this. */
export const toolErrorPath = (plugin: string, tool: string): string => `${ZIP_FOLDERS.errors}${plugin}/${tool}.json`;
...
export interface ToolNeuronOptions {
  fireRatio?: number;
  minEnergy?: number;
  maxResultChars?: number;
  maxPending?: number;
  /**
   * The lock over this engine's doorways. Share the one the chat feed holds
   * (PromptMeshFeed.lock()): two streams mid-settle on one engine at once
   * corrupt each other.
   */
  lock?: DoorwayLock;
  /** How to give the event loop a turn between result bytes. Injectable so a test can run straight through. */
  yieldTo?: () => Promise<void>;
  /**
   * Checked before any call the NETWORK makes. Without it, a fired neuron
   * runs its tool unchecked -- fine for a test mesh, never for the live one,
   * which passes the shared AccessManager.
   */
  access?: ToolAccessCheck;
}

export interface ToolNeuronStats {
  /** Rising edges seen on tool neurons. */
  fired: number;
  /** Calls the network's firings turned into. */
  networkCalls: number;
  /** Calls from anywhere else (chat, code) that were turned into neuron events. */
  otherCalls: number;
  /** Results streamed all the way in. */
  resultsFed: number;
  /** Bytes of packed results that went through result channels. */
  bytesFed: number;
  /** Results dropped because too many were waiting. */
  dropped: number;
  lastError: string | null;
}

/** Neurons already given to a plugin, per engine: attaching twice must not keep growing the network. */
const allocations = new WeakMap<HyperDimensionalEngine, Map<string, { tools: Record<string, number>; channel: ResultChannel }>>();

function allocationsFor(engine: HyperDimensionalEngine) {
...
function clip(value: unknown, max: number): { value: unknown; truncated?: number } {
  let text: string;
  try {
  ...
  if (text.length <= max) return { value: typeof value === "string" ? value : safeParse(text) };
...
export class ToolNeuronLayer {
  private readonly neurons = new Map<string, ToolNeuron>();
  private readonly channels = new Map<string, ResultChannel>();
  private readonly sources = new Map<string, NeuronToolSource>();
  ...
  private readonly latched = new Map<string, ToolFiring>();
  /** >0 while this layer is driving a tool neuron itself: that tick is the call being felt, not the network deciding. */
  private selfDriving = 0;

  private readonly calls: ToolCallEvent[] = [];
  private readonly lock: DoorwayLock;
  private readonly yieldTo: () => Promise<void>;
  /** Results waiting for the doorway, oldest first. */
  private readonly queue: Array<{ event: ToolCallEvent; neuronId?: number; channel: ResultChannel }> = [];
  private pumping: Promise<void> | null = null;
  private readonly stats: ToolNeuronStats = {
    fired: 0, networkCalls: 0, otherCalls: 0, resultsFed: 0, bytesFed: 0, dropped: 0, lastError: null,
  };

  private readonly fireRatio: number;
  private readonly minEnergy: number;
  private readonly maxResultChars: number;
  private readonly maxPending: number;
  private readonly access?: ToolAccessCheck;

  constructor(private readonly engine: HyperDimensionalEngine, options: ToolNeuronOptions = {}) {
   ...
   */
  attach(source: NeuronToolSource): AttachResult {
    ...
    if (!allocation) {
      ...
      const channel: ResultChannel = { plugin, bit0In: ids[specs.length], bit1In: ids[specs.length + 1] };
  ...
  layout(): { tools: Array<Omit<ToolNeuron, "spec"> & { description: string }>; channels: ResultChannel[] } {
    return {
  ...
  neuronFor(plugin: string, tool: string): number | undefined {
    return this.neurons.get(`${plugin}.${tool}`)?.neuronId;
  }

  channelFor(plugin: string): ResultChannel | undefined {
  ...
  private observeTick(): void {
    ...
    for (const neuron of this.neurons.values()) {
      ...
      if (this.lit.has(neuron.key)) continue;
      ...
      if (this.selfDriving > 0) continue;
      this.stats.fired++;
  ...
  decide(): ToolDecision {
    const line = this.firingLine();
    const options: ToolOption[] = [...this.neurons.values()].map(neuron => {
      const energy = this.engine.getNeuronEnergy(neuron.neuronId);
   ...
   */
  async step(output: ZipTree | null = null): Promise<ToolCallEvent[]> {
  ...
  private argumentsFor(output: ZipTree | null, plugin: string, tool: string): Record<string, unknown> | string {
  ...
  private refuse(firing: ToolFiring, error: string, args: Record<string, unknown> = {}): ToolCallEvent {
    const now = Date.now();
   ...
   */
  async call(plugin: string, tool: string, args: Record<string, unknown> = {}): Promise<ToolCallEvent> {
    const source = this.sources.get(plugin);
    if (!source) throw new Error(`No plugin "${plugin}" is attached to the network's tool layer.`);
   ...
   */
  private record(event: ToolCallEvent): void {
    const neuron = this.neurons.get(`${event.plugin}.${event.tool}`);
    const channel = this.channels.get(event.plugin);
    this.calls.push(event);
    if (this.calls.length > HISTORY_LIMIT) this.calls.splice(0, this.calls.length - HISTORY_LIMIT);
    if (event.origin === "network") this.stats.networkCalls++;
    else this.stats.otherCalls++;
    if (!channel) return;
      ...
      while (this.queue.length > 0) {
        ...
        await this.lock.run(async () => {
          try {
            ...
            if (job.neuronId !== undefined && job.event.origin !== "network") this.driveOnce(job.neuronId);
  ...
  packResult(event: ToolCallEvent): Uint8Array {
  ...
  private driveOnce(neuronId: number): void {
    this.selfDriving++;
   ...
   */
  private async stream(channel: ResultChannel, bytes: Uint8Array): Promise<void> {
    ...
    if (last) this.engine.process(pulse, undefined, last);
  ...
  private pulse(): number[] {
    if (!this.pulseScratch) this.pulseScratch = new Array(this.engine.getDimensions()).fill(1);
