...
import { publishAndSync, assertSafeName, type StoreFile } from "./store.js";
...
export interface NetSkillPublishResult {
  skill: string;
  slug: string;
  neurons: number;
 ...
 */
export async function publishGraftedNetSkills(
  engine: HyperDimensionalEngine,
  opts: { author?: string } = {},
): Promise<NetSkillPublishResult[]> {
  const skills = graftedSkills(engine);
  const results: NetSkillPublishResult[] = [];
  ...
  for (const { skill, ids } of skills) {
    ...
    if (neuronNames.length === 0) continue;
    ...
    const { sync } = await publishAndSync({
      ...
      title: skill,
      description:
        `A Net Skill grafted into the live mesh: ${neuronNames.length} neuron` +
