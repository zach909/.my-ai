...
export class ZipIOLoop {
  ...
  private inputLoop: LoopZipChunk[];
  private outputLoop: LoopZipChunk[];
  private inputPosition: number;
  private outputPosition: number;
  private stats: IOLoopStats;
   ...
   */
  async writeInput(data: string | Buffer): Promise<string> {
   ...
   */
  async readInput(chunkId?: string): Promise<Buffer | null> {
    ...
    if (chunkId) {
      ...
      if (this.inputLoop.length === 0) return null;
   ...
   */
  async writeOutput(data: string | Buffer): Promise<string> {
    const buffer = Buffer.isBuffer(data) ? data : Buffer.from(data);
   ...
   */
  async readOutput(chunkId?: string): Promise<Buffer | null> {
    this.stats.readCount++;
    ...
    if (chunkId) {
      ...
      // Read current position
      if (this.outputLoop.length === 0) return null;
   ...
   */
  async readAllInput(): Promise<Buffer[]> {
   ...
   */
  async readAllOutput(): Promise<Buffer[]> {
    const results: Buffer[] = [];
   ...
   */
  async concatenateInput(): Promise<Buffer> {
   ...
   */
  async concatenateOutput(): Promise<Buffer> {
   ...
   */
  private async decompressChunk(chunk: LoopZipChunk): Promise<Buffer> {
    if (chunk.compressed) {
   ...
   */
  getInputChunk(chunkId: string): LoopZipChunk | undefined {
   ...
   */
  getOutputChunk(chunkId: string): LoopZipChunk | undefined {
