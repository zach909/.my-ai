...
`LongTermMemory.consolidateFrom(texts[])` — a batch version of `remember()` intended (per its own doc comment) for "working-context snippets drained from the ZipIO buffer" — remains genuinely unused: `ZipIOSystem` has no method that returns its buffered chunks as a plain string array to feed it, and forcing that integration in the same pass risked exactly the "cosmetic no-op or rushed, likely-incomplete implementation" this document has deliberately avoided elsewhere (see "Identified but deferred: 'modify an existing skill'" above). Recorded here honestly as a real, smaller, still-open gap rather than closed under time pressure.
