...
This isn't just a memory leak. `observe()` sits on the two busiest real entry points in the system: `index.ts`'s `processQueryImpl()` calls `this.monitor.observe("prediction.surprise", ...)` on *every chat message* through the CLI or web server, and `solve()` calls `this.monitor.observe("solve.confidence", ...)` on every call. `anomalies()` does a full linear scan of `log` (`for (const o of this.log) latest.set(o.name, o)`), and `hasFailure()` — called right after both `observe()` sites above, on every single one of those calls — invokes `anomalies()` internally. So both memory *and* per-call CPU cost were growing unboundedly with total lifetime query count on the busiest paths in the whole system, on a process-lifetime singleton (`getNeuroclawSystem()`).
