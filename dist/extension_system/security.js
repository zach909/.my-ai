...
export class PermissionDeniedError extends Error {
    constructor(extensionId, permission) {
...
export class PermissionGuard {
    ...
    grant(extensionId, permission, grantedBy = "user") {
        const g = { extensionId, permission, granted: true, grantedAt: Date.now(), grantedBy };
        this.forId(extensionId).set(permission, g);
        return g;
    }
    revoke(extensionId, permission) {
        this.forId(extensionId).delete(permission);
    }
    isGranted(extensionId, permission) {
        return this.forId(extensionId).get(permission)?.granted === true;
    }
    listGrants(extensionId) {
     ...
     */
    autoGrantNonSensitive(extensionId, permissions) {
        const stillNeeded = [];
    ...
    assertAll(extensionId, permissions) {
        for (const p of permissions) {
            if (!this.isGranted(extensionId, p))
