...
import { ACCESS_SWITCHES, AccessManager, CAPABILITIES, CAPABILITY_MINIMUM, CAPABILITY_SWITCH, SWITCH_DESCRIPTION, SWITCH_LABEL, defaultGrants, } from "./access-manager.js";
...
export function loadSettings(file = accessSettingsPath()) {
    if (!existsSync(file))
    ...
    try {
        ...
        const grants = (Array.isArray(raw.grants) ? raw.grants : []).filter((g) => Boolean(g) && CAPABILITIES.includes(g.capability) && typeof g.level === "string");
...
export function saveSettings(settings, file = accessSettingsPath()) {
 ...
 */
export function loadAccessManager(file = accessSettingsPath()) {
...
export function describeAccess(manager) {
    ...
    const capabilities = CAPABILITIES.map(capability => {
        ...
        const blockedBySwitch = !state.all ? "all" : state[group] ? null : group;
        return {
            capability,
