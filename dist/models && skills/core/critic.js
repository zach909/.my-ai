...
export class Critic {
    ...
    async review(input) {
        ...
        const calculationChecks = (input.calculations ?? []).map((c) => {
        ...
        const assumptionChallenges = (input.assumptions ?? []).map((a) => {
        ...
        for (const ac of assumptionChallenges) {
            if (!ac.supported)
                issues.push(`unchallenged assumption: "${ac.assumption}"`);
        }
        ...
        if (this.deps.knowledge) {
            ...
            if (contradictions.length)
                issues.push(`${contradictions.length} contradiction(s) found in knowledge graph`);
        }
        ...
        if (this.deps.testCode) {
            for (const code of input.code ?? []) {
                ...
                if (!r.passed)
                    issues.push(`code test failed: ${r.detail}`);
            }
        }
        const passed = calculationChecks.every((c) => c.verified) &&
    ...
    async compareAnswers(candidates) {
        ...
        for (const c of candidates)
            reports.set(c.id, await this.review(c));
     ...
     */
    static fromReasoningResult(result, extra = {}) {
        return {
            claim: result.result,
            task: result.problem,
