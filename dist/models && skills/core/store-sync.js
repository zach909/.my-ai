...
async function repoRoot(storeDir) {
 ...
 */
async function attemptSync(root, remote, branch, relPaths, message, tmpIndex) {
    ...
    if (diff.ok)
        return { pushed: true, changed: false };
 ...
 */
export async function syncStorePaths(paths, message, opts) {
    ...
    try {
        ...
        if (!first.changed) {
            if (first.error)
                return { committed: false, pushed: false, branch, reason: first.error };
