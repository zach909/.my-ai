 ...
 */
async function repoRoot(startDir) {
 ...
 */
export async function pullStoreCatalog(opts = {}) {
    ...
    if (!root)
        return { pulled: false, reason: "Not a git repository." };
    ...
    try {
        ...
        const archiveResult = await new Promise(resolve => {
        ...
        if (!extracted)
            return { pulled: false, reason: "Could not extract the store branch's contents." };
    }
    finally {
        try {
...
export async function resolveRawUrlBase() {
    ...
    const refs = [branch];
    const checkedOut = (await git(["symbolic-ref", "--short", "HEAD"], start)) ??
        (await git(["rev-parse", "--abbrev-ref", "HEAD"], start)) ??
        "";
    if (checkedOut && checkedOut !== "HEAD" && !refs.includes(checkedOut))
        refs.push(checkedOut);
    const head = await git(["symbolic-ref", "--short", "refs/remotes/origin/HEAD"], start);
    const defaultBranch = head?.replace(/^origin\//, "");
    if (defaultBranch && !refs.includes(defaultBranch))
...
export async function rawUrlBase() {
 ...
 */
export async function fetchItemFile(kind, name, filename) {
    assertKind(kind);
    assertSafeName(name);
    assertSafeFilename(filename);
    const item = readItem(kind, name);
    if (!item)
        throw new StoreFetchError(`There is no published "${kind}/${name}".`);
    const entry = item.files.find(f => f.filename === filename);
    if (!entry)
        throw new StoreFetchError(`"${filename}" is not part of ${kind}/${name}.`);
    ...
    if (entry.bytes > MAX_FETCH_BYTES) {
        throw new StoreFetchError(`"${filename}" is ${(entry.bytes / 1024 / 1024).toFixed(1)} MB, over the ${MAX_FETCH_BYTES / 1024 / 1024} MB limit.`);
    }
    const resolved = await resolveRawUrlBase();
    if (!("base" in resolved)) {
    ...
    for (const refBase of resolved.refs) {
        ...
        try {
            ...
            throw new StoreFetchError(`Could not reach GitHub to download "${filename}": ${err instanceof Error ? err.message : String(err)}`);
        }
    }
    if (!res) {
        throw new StoreFetchError(`GitHub returned ${lastStatus} for "${filename}" on every branch tried (${resolved.refs.length}). It may not have been pushed yet.`);
    }
    ...
    if (entry.sha256 && got !== entry.sha256) {
        throw new StoreFetchError(`"${filename}" did not match its published checksum, so it was not saved. Expected ${entry.sha256.slice(0, 16)}…, got ${got.slice(0, 16)}….`);
    }
