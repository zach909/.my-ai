 ...
 */
import { graftNetSkill } from "./net-skill-graft.js";
/**
 ...
 */
export function buildDomainSkills(engine) {
    return DOMAIN_SKILLS.map(domain => {
        const neurons = domain.concepts.map(concept => ({
            name: `${domain.category}:${concept.replace(/\s+/g, "-")}`,
            definition: concept,
        }));
        const result = graftNetSkill(engine, domain.title, neurons);
        return {
...
export function totalDomainNeurons() {
