...
export class ChatGroup {
    constructor(id, name, hive) {
         ...
         */
        this.messagesCapacity = 5000;
        this.id = id;
        this.name = name;
        this.hive = hive;
    }
    addMember(agentId) {
        if (!this.hive.get(agentId))
            throw new Error(`No such hive agent: ${agentId}`);
    ...
    async decide(question, options) {
        if (options.length === 0)
            throw new Error("decide requires at least one option");
