...
import { assertKind, assertSafeName, readItem, readItemFile } from "./store.js";
...
function itemInstallDir(kind, name) {
 ...
 */
export async function installItem(kind, name) {
    const item = readItem(kind, name);
    if (!item)
        throw new StoreInstallError(`There is no published "${kind}/${name}" to install.`);
    if (item.files.length === 0)
        throw new StoreInstallError(`"${kind}/${name}" has no files to install.`);
    ...
    if (written.length === 0) {
        throw new StoreInstallError(`Could not install "${kind}/${name}": none of its files could be obtained. ${missing[0]?.reason ?? ""}`.trim());
    }
    const record = {
        kind: item.kind,
        name: item.name,
 ...
 */
export function uninstallItem(kind, name) {
...
export function readInstalled(kind, name) {
...
export function listInstalledItems() {
    ...
    for (const kind of readdirSync(root)) {
        ...
        for (const name of readdirSync(kindDir)) {
            if (!statSync(path.join(kindDir, name)).isDirectory())
                continue;
...
export function changedFiles(record, published) {
    ...
    for (const filename of installed.keys()) {
        if (!published.files.some(f => f.filename === filename))
            changed.push(filename);
    }
    return changed.sort();
}
export function outdatedInstalls() {
    const out = [];
...
export function readInstalledFile(kind, name, filename) {
    const dir = itemInstallDir(kind, name);
 ...
 */
export function planActivation(kind, name) {
    const record = readInstalled(kind, name);
    if (!record) {
    ...
    for (const file of record.files) {
        ...
        if (!/\.(skill|source|ext)\.json$/i.test(file.filename) && !/^skill\.json$/i.test(file.filename))
            continue;
        const buf = readInstalledFile(kind, name, file.filename);
        if (!buf)
            continue;
        ...
        for (const raw of neurons) {
            if (memories.length >= MAX_MEMORIES_PER_ITEM)
                break;
            const neuron = raw;
            if (!neuron?.name)
                continue;
    ...
    if (memories.length === 0 && meshNeurons.length === 0) {
        return {
            ...
            memories,
            neurons: meshNeurons,
            nothingLoadable: `"${kind}/${name}" is installed, but carries nothing this system knows how to load ` +
    ...
    return {
        ...
        ...(hitCap
            ? {
                truncated: `"${kind}/${name}" declares more than the ${MAX_MEMORIES_PER_ITEM} memories one item may contribute. ` +
