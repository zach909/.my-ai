...
export class SharedChatStore {
    ...
    readRoomsIndex() {
        if (!existsSync(this.roomsIndexFile))
            return [];
        try {
    ...
    listRooms() {
        ...
        return [...rooms].sort((a, b) => {
            if (a.id === GENERAL_ROOM_ID)
                return -1;
            if (b.id === GENERAL_ROOM_ID)
                return 1;
    ...
    ensureGeneralRoom() {
        const rooms = this.readRoomsIndex();
        if (rooms.some((r) => r.id === GENERAL_ROOM_ID))
            return;
    ...
    ensureRoom(name) {
        ...
        const room = { id, name: trimmed, createdAt: Date.now(), lastMessageAt: null };
        rooms.push(room);
        this.writeRoomsIndex(rooms);
        return room;
    }
    ...
    post(roomId, author, text, isBot = false) {
...
export function getSharedChatStore() {
    if (!sharedInstance)
        sharedInstance = new SharedChatStore();
