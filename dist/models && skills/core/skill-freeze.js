 ...
 */
import { planRequirements } from "./requirement-planner.js";
...
export function freezeMainModel(view) {
...
export class MainModelChanged extends Error {
    constructor(before, after) {
        super(`The main model changed while a skill was being built against it (${before} -> ${after}). ` +
...
export function assertMainModelUnchanged(frozen, view) {
    const now = digestOf(view.neuronNames());
    if (now !== frozen.digest)
        throw new MainModelChanged(frozen.digest, now);
}
 ...
 */
function findInFrozenModel(frozen) {
 ...
 * from either.
 */
export function planAgainstFrozenModel(requirements, view, context = {}) {
