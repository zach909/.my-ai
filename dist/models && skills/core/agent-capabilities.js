...
import { attachmentMetadata } from "./chat-attachments.js";
/**
 ...
 */
function extractAttachmentMarkers(text, sink) {
    return text.replace(/\s*\[\[ATTACH:([0-9a-fA-F-]+)\]\]/g, (_match, id) => {
 ...
 */
export function buildAgentCapabilities(host, attachmentsOut = []) {
    ...
    if (!host.research && host.pluginRegistry) {
        const instance = host.pluginRegistry.getPluginInstance("research");
    ...
    const caps = {
        decompose: (goal) => decompose(goal, 4),
    };
    if (host.memory) {
        caps.recall = (query) => host
            .memory.retrieve(query, { topK: PERCEPTION_LIMIT })
            .map(hit => hit.item.payload ?? hit.item.content)
            .filter(text => typeof text === "string" && text.trim().length > 0);
    }
    if (host.mistakes) {
        caps.lessons = (task) => host.mistakes.lessons(task, PERCEPTION_LIMIT);
    }
    caps.searchWiki = (query) => {
        try {
    ...
    if (host.pluginRegistry) {
        caps.callPlugin = async (pluginId, input) => {
    ...
    if (host.pluginRegistry?.rankPlugins) {
         ...
         * right tool".
         */
        caps.findPlugin = (task) => host.pluginRegistry.rankPlugins(task).slice(0, 3);
        /**
    ...
    caps.isGoalMet = (_goal, observations) => {
 ...
 */
const REACHES_BEYOND = new Set(["web", "chats"]);
export async function runAgentLoopForMessage(userMessage, host) {
    ...
    if (actions.length === 0 && reaching.length === 0)
        return null;
    const attachments = [];
    ...
    if (reaching.length > 0) {
        ...
        if (text.length > 0)
            return { answered: true, message: text, result, attachments };
    }
