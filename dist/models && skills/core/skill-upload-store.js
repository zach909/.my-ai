...
import { syncStorePaths } from "./store-sync.js";
...
export const SKILL_UPLOAD_SLOTS = ["plugin", "sourceSkill", "binarySkill", "algorithm", "rsiTest"];
...
function readManifest(name) {
    ...
    try {
        ...
        if (raw && typeof raw === "object" && ("slots" in raw || "extraFiles" in raw || "wikiPage" in raw)) {
            ...
            if (typeof raw.wikiPage === "string")
                manifest.wikiPage = raw.wikiPage;
...
export function listSkillUploads() {
...
export function readSkillUpload(name) {
    assertSafeName(name);
    if (!existsSync(packageDir(name)))
        return null;
    return { name, ...readManifest(name) };
}
export function readSkillUploadFile(name, slot) {
    assertSafeName(name);
...
export function readSkillUploadExtraFile(name, filename) {
    assertSafeName(name);
    const manifest = readManifest(name);
    if (!manifest.extraFiles.some((f) => f.filename === filename))
        return null;
 ...
 */
export function saveSkillUpload(name, files) {
    assertSafeName(name);
    const entries = Object.entries(files).filter(([, f]) => f !== undefined);
    if (entries.length === 0) {
 ...
 */
export function saveSkillUploadExtraFiles(name, files) {
    assertSafeName(name);
    if (files.length === 0) {
        throw new SkillUploadError("Provide at least one file to upload.");
    }
    ...
    for (const file of files) {
        ...
        writeFileSync(path.join(dir, file.filename), file.content, "utf8");
        const entry = { filename: file.filename, uploadedAt: Date.now(), bytes: Buffer.byteLength(file.content, "utf8") };
        const existingIndex = manifest.extraFiles.findIndex((f) => f.filename === file.filename);
        if (existingIndex >= 0)
            manifest.extraFiles[existingIndex] = entry;
        else
            manifest.extraFiles.push(entry);
    }
    writeManifest(name, manifest);
    return { name, ...manifest };
}
 ...
 * registry it feeds.
 */
export function linkSkillUploadWiki(name, wikiPageName) {
    assertSafeName(name);
    const dir = packageDir(name);
    if (!existsSync(dir))
        mkdirSync(dir, { recursive: true });
...
export function unlinkSkillUploadWiki(name) {
    assertSafeName(name);
    if (!existsSync(packageDir(name))) {
...
export function recordSkillUploadRsiPass(name, message) {
    assertSafeName(name);
    if (!existsSync(packageDir(name))) {
        throw new SkillUploadError(`No skill package named "${name}".`);
    }
...
export function deleteSkillUploadExtraFile(name, filename) {
    assertSafeName(name);
    const manifest = readManifest(name);
...
export function deleteSkillUpload(name) {
    assertSafeName(name);
    const dir = packageDir(name);
    if (!existsSync(dir)) {
        throw new SkillUploadError(`No skill package named "${name}".`);
 ...
 */
export async function saveSkillUploadAndSync(name, files) {
...
export async function saveSkillUploadExtraFilesAndSync(name, files) {
...
export async function deleteSkillUploadAndSync(name) {
    const dir = packageDir(name);
 ...
 */
export async function linkSkillUploadWikiAndSync(name, wikiPageName) {
...
export async function unlinkSkillUploadWikiAndSync(name) {
...
export async function recordSkillUploadRsiPassAndSync(name, message) {
...
export async function deleteSkillUploadExtraFileAndSync(name, filename) {
