 ...
 */
const grafted = new WeakMap();
function registryFor(engine) {
 ...
 * arrive in step.
 */
export function waveForMeaning(text) {
    let hash = 0;
    let mix = 0;
    ...
    const spread = (Math.abs(hash) % 100000) / 100000;
    const turn = (Math.abs(mix) % 100000) / 100000;
    return {
 ...
 */
export function graftNetSkill(engine, skillName, neurons) {
    ...
    const named = neurons.filter((n) => typeof n?.name === "string" && n.name.trim().length > 0);
    if (named.length === 0) {
    ...
    for (const neuron of named) {
        ...
        for (const [targetName, weight] of connectionsOf(neuron)) {
            ...
            if (to === undefined)
                continue; // names something outside the skill
            if (engine.setConnection(to, from, weight))
                connections++;
        }
    }
...
export function graftedSkills(engine) {
