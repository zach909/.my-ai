...
export class PromptingSkill {
    constructor(intentRouter, plan, empathy) {
    ...
    understand(input, reason) {
        ...
        if (reason)
            this.plan.setReason(reason);
        if (subtasks.length > 1)
            this.plan.addSteps(subtasks);
        else if (trimmed)
            this.plan.addStep(trimmed);
        return {
            objective: trimmed,
            missingInfo,
            subtasks,
     ...
     */
    shouldProceedAutonomously(analysis) {
