 ...
 */
function backupBeforeChange(name) {
...
export function extractWikiSummary(raw) {
...
function assertSafeName(name) {
    if (!SAFE_NAME.test(name)) {
        throw new WikiNameError(`"${name}" is not a valid wiki page name -- letters, digits, '-', and '_' only (no '.' or '/').`);
    }
}
...
export function readWikiPage(name) {
    assertSafeName(name);
 ...
 * depend on.
 */
export function publishWikiPage(name, title, content) {
    assertSafeName(name);
    if (!title.trim())
        throw new WikiNameError("A wiki page needs a non-empty title.");
    if (!content.trim())
        throw new WikiNameError("A wiki page needs non-empty content.");
    ...
    if (existsSync(path.join(wikiDir(), `${name}.md`))) {
        throw new WikiNameError(`"${name}" is already a curated wiki page and can't be overwritten here -- pick a different name.`);
    }
 ...
 */
export function deleteWikiPage(name) {
    assertSafeName(name);
    if (existsSync(path.join(wikiDir(), `${name}.md`))) {
...
export function listWikiBackups(name) {
    assertSafeName(name);
    const dir = backupsDir(name);
    if (!existsSync(dir))
        return [];
...
export function readWikiBackup(name, timestamp) {
    assertSafeName(name);
 ...
 */
export function restoreWikiBackup(name, timestamp) {
 ...
 */
export async function publishWikiPageAndSync(name, title, content) {
    const page = publishWikiPage(name, title, content);
    const sync = await syncStorePaths([path.join(botWikiDir(), `${name}.md`)], `wiki: publish ${name}`, { storeDir: storeRoot() });
    return { page, sync };
}
/** Delete a bot-published page and propagate the removal, so a pull cannot resurrect it. */
export async function deleteWikiPageAndSync(name) {
    const file = path.join(botWikiDir(), `${name}.md`);
    deleteWikiPage(name);
