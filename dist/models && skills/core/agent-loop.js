 ...
 */
import { fillTemplate, } from "./prompting-skills.js";
 ...
 */
async function perceive(skill, goal, last, caps) {
    ...
    switch (skill.source) {
        ...
        case "plugin": {
            if (!caps.callPlugin || !skill.plugin)
                return [];
...
async function think(skill, goal, observations, caps) {
 ...
 */
export async function runAgentLoop(goal, registry, caps = {}, opts = {}) {
    const maxIterations = Math.max(1, opts.maxIterations ?? DEFAULT_MAX_ITERATIONS);
    const steps = [];
    const observations = [];
    let iteration = 0;
    let outcome = "max-iterations";
    while (iteration < maxIterations) {
        ...
        if (acted && actionResult)
            observations.push(actionResult);
