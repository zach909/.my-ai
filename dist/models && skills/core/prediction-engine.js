...
export class PredictionEngine {
    ...
    predict(action, opts = {}) {
        ...
        for (const o of outcomes)
            o.likelihood = total > 0 ? o.likelihood / total : 1 / outcomes.length;
...
function inferAssumptions(action) {
    const assumptions = [];
    const t = action.toLowerCase();
    if (/\b(file|directory|folder)\b/.test(t))
        assumptions.push("the referenced file/directory exists and is accessible");
    if (/\b(network|api|service|server)\b/.test(t))
        assumptions.push("the network/service is reachable");
    if (/\b(delete|remove|overwrite)\b/.test(t))
        assumptions.push("no other process depends on the current state");
