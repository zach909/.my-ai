...
export class AgentWorkspace {
    ...
    openTerminal(id, cwd) {
        ...
        if (this.sessions.has(id))
            throw new WorkspaceError(`A terminal called "${id}" is already open.`);
     ...
     * process.
     */
    readTerminal(id, lines = 50) {
        const session = this.sessions.get(id);
        if (!session)
            throw new WorkspaceError(`No terminal called "${id}".`);
     ...
     */
    async run(id, command, args = [], opts = {}) {
        this.guard("terminal.execute", () => this.access.require("terminal.execute"));
        const session = this.sessions.get(id);
        if (!session)
            throw new WorkspaceError(`No terminal called "${id}". Open it first.`);
        if (session.running)
            throw new WorkspaceError(`"${id}" is busy running: ${session.running}`);
        ...
        return await new Promise(resolve => {
            ...
            const capped = (current, addition) => {
                const room = MAX_CAPTURED_BYTES - current.length;
                if (room <= 0)
                    return { text: current, dropped: addition.length };
                if (addition.length <= room)
                    return { text: current + addition, dropped: 0 };
    ...
    listDirectory(target) {
        ...
        if (!existsSync(full) || !statSync(full).isDirectory())
            throw new WorkspaceError(`"${full}" is not a directory.`);
    ...
    readFile(target) {
        ...
        if (!existsSync(full) || !statSync(full).isFile())
            throw new WorkspaceError(`"${full}" is not a file.`);
    ...
    deleteFile(target) {
        ...
        if (!existsSync(full))
            throw new WorkspaceError(`"${full}" does not exist.`);
