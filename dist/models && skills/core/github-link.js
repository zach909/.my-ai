...
function git(args, cwd) {
    return new Promise(resolve => {
        execFile("git", args, { cwd, timeout: GIT_TIMEOUT_MS, maxBuffer: 1024 * 1024, env: { ...process.env, GIT_TERMINAL_PROMPT: "0" } }, (err, stdout, stderr) => resolve({ ok: !err, stdout: String(stdout ?? ""), stderr: String(stderr ?? "") }));
    });
}
/**
 ...
 */
export function githubRepoSlug(remoteUrl) {
...
async function slugFor(cwd, remote) {
    const res = await git(["remote", "get-url", remote], cwd);
    if (!res.ok)
        return { reason: `No "${remote}" remote to read a URL from.` };
    const slug = githubRepoSlug(res.stdout);
    if (!slug)
        return { reason: `The "${remote}" remote is not a github.com repository.` };
    return { slug };
}
/**
 ...
 */
export async function githubFileUrl(absPath, branch, opts = {}) {
    ...
    if (!root)
        return { url: null, reason: "Not inside a git repository." };
    ...
    if ("reason" in found)
        return { url: null, reason: found.reason };
...
export async function githubDirUrl(absDir, branch, opts = {}) {
    const remote = opts.remote ?? "origin";
