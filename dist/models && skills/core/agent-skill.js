...
export function isAgentSkillFile(filename) {
 ...
 */
export function parseAgentSkill(text, fallbackName = "") {
    ...
    if (!name)
        throw new AgentSkillError("A SKILL.md needs a name, in frontmatter or as its first heading.");
    if (!description) {
    ...
    if (!body)
        throw new AgentSkillError(`"${name}" has frontmatter but no instructions.`);
    if (!description) {
        ...
        throw new AgentSkillError(`"${name}" has no description, so there is nothing to say when it applies. ` +
