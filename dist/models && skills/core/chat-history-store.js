...
import { ChatOrganizer } from "./chat-organizer.js";
const TITLE_MAX = 80;
...
export class ChatHistoryStore {
    ...
    listThreads() {
        ...
        for (const entry of readdirSync(this.rootDir)) {
            if (!entry.endsWith(".json"))
                continue;
            try {
                threads.push(JSON.parse(readFileSync(join(this.rootDir, entry), "utf8")));
            }
            catch {
    ...
    deleteAllThreads() {
        ...
        for (const thread of this.listThreads()) {
            if (this.deleteThread(thread.id))
                deleted++;
        }
        return deleted;
    }
     ...
     */
    appendMessage(message, source, threadId) {
        this.ensureDir();
    ...
    search(query, opts = {}) {
        ...
        const results = [];
        for (const thread of this.listThreads()) {
            if (thread.id === opts.excludeId)
                continue;
            if (opts.source && thread.source !== opts.source)
                continue;
            let best = { score: 0, snippet: "" };
            for (const m of thread.messages) {
                const score = tokenOverlap(q, tokenize(m.content));
                if (score > best.score)
                    best = { score, snippet: m.content.slice(0, 200) };
            }
            if (best.score > 0.15)
                results.push({ thread, score: best.score, snippet: best.snippet });
        }
