...
import { builtInPromptingSkills, parsePromptingSkill, PromptingSkillError, PromptingSkillRegistry, } from "./prompting-skills.js";
import { publishAndSync, readItemFile, storeRoot } from "./store.js";
...
export async function publishPromptingSkill(raw) {
...
export function readPublishedPromptingSkill(name) {
    ...
    try {
        ...
        throw new PromptingSkillError(`"${name}" is published but is not a valid prompting skill: ${err instanceof Error ? err.message : String(err)}`);
    }
}
 ...
 * use from now on".
 */
export function installPromptingSkill(raw) {
    const skill = parsePromptingSkill(raw);
    const dir = installedDir();
    if (!existsSync(dir))
...
export function installFromStore(name) {
...
export function uninstallPromptingSkill(name) {
...
export function loadRegistry() {
    ...
    for (const skill of builtInPromptingSkills())
        registry.install(skill);
    for (const skill of listInstalled())
        registry.install(skill);
    return registry;
}
...
export function promptingStoreDir() {
