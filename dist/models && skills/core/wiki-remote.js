...
import { extractWikiSummary } from "./wiki-store.js";
const GIT_TIMEOUT_MS = 15000;
function git(args, cwd) {
    return new Promise(resolve => {
        execFile("git", args, { cwd, timeout: GIT_TIMEOUT_MS, maxBuffer: 4 * 1024 * 1024, env: { ...process.env, GIT_TERMINAL_PROMPT: "0" } }, (err, stdout) => resolve({ ok: !err, stdout: String(stdout ?? "") }));
    });
}
 ...
 */
async function remoteRef(remote, branch) {
 ...
 */
async function remoteBotPageNames(remote, branch) {
 ...
 */
export async function listRemoteOnlyBotPages(excludeNames) {
 ...
 */
export async function readRemoteBotPage(name) {
    const remote = process.env.NEUROCLAW_STORE_REMOTE?.trim() || "origin";
    const branch = DEFAULT_STORE_BRANCH;
    const target = await remoteRef(remote, branch);
    if (!target)
