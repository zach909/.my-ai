...
import { publishAndSync, readItem } from "./store.js";
 ...
 */
export function changedAgainstStore(kind, name, artifacts) {
    ...
    for (const artifact of artifacts) {
        ...
        if (!published || published !== sha256Hex(artifact.content))
            changed.push(artifact.filename);
    }
    return changed;
}
 ...
 */
export function describeAutonomousSkill(topic, verifiedSources) {
    return [
