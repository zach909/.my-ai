...
export class MistakeTracker {
    ...
    record(input) {
        ...
        if (existing) {
            ...
            if (input.prevention)
                existing.prevention = input.prevention;
            if (input.assumption)
    ...
    similar(task, topK = 5) {
        ...
        for (const m of this.mistakes.values()) {
            ...
            const seen = new Set();
            for (const w of t)
                if (q.has(w) && !seen.has(w)) {
                    overlap++;
                    seen.add(w);
                }
            if (overlap > 0)
                scored.push({ m, score: overlap / Math.sqrt(q.size * (new Set(t).size || 1)) });
        }
    ...
    lessons(task, topK = 5) {
        return this.similar(task, topK).map(m => m.prevention).filter((p) => !!p);
    }
