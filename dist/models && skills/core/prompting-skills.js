...
export const PROMPTING_CATEGORIES = ["perception", "cognitive", "action"];
export const PROMPTING_CATEGORY_LABELS = {
 ...
 */
export const PERCEPTION_SOURCES = ["memory", "wiki", "store", "chats", "web", "plugin"];
/**
 ...
 */
export const COGNITIVE_STRATEGIES = ["decompose", "recall-lessons", "compare-options", "plan-next-step"];
...
function str(value, field, max = MAX_TEXT) {
    if (typeof value !== "string")
        throw new PromptingSkillError(`"${field}" must be a string.`);
    const trimmed = value.trim();
    if (trimmed.length > max)
        throw new PromptingSkillError(`"${field}" is longer than ${max} characters.`);
    return trimmed;
}
/**
 ...
 * quietly never fires.
 */
export function parsePromptingSkill(raw) {
    if (!raw || typeof raw !== "object")
        throw new PromptingSkillError("A prompting skill must be a JSON object.");
    const o = raw;
    const name = str(o.name, "name", 64);
    if (!NAME_RE.test(name) || name.includes("..")) {
        throw new PromptingSkillError(`"${name}" is not a valid name — letters, digits, '.', '-' and '_' only, up to 64 characters.`);
    }
    const category = str(o.category, "category", 32);
    if (!PROMPTING_CATEGORIES.includes(category)) {
        throw new PromptingSkillError(`"${category}" is not a category. Use one of: ${PROMPTING_CATEGORIES.join(", ")}.`);
    }
    const whenRaw = o.when ?? [];
    if (!Array.isArray(whenRaw))
        throw new PromptingSkillError(`"when" must be an array of strings.`);
    if (whenRaw.length > MAX_WHEN)
        throw new PromptingSkillError(`"when" has more than ${MAX_WHEN} entries.`);
    ...
    if (category === "perception") {
        const source = str(o.source, "source", 32);
        if (!PERCEPTION_SOURCES.includes(source)) {
            throw new PromptingSkillError(`"${source}" is not a perception source. Use one of: ${PERCEPTION_SOURCES.join(", ")}.`);
        }
        skill.source = source;
        ...
        const strategy = str(o.strategy, "strategy", 32);
        if (!COGNITIVE_STRATEGIES.includes(strategy)) {
            throw new PromptingSkillError(`"${strategy}" is not a strategy. Use one of: ${COGNITIVE_STRATEGIES.join(", ")}.`);
        }
        ...
        if (o.expect !== undefined)
            skill.expect = str(o.expect, "expect");
    }
    return skill;
}
 ...
 */
export function skillApplies(skill, goal, observation) {
    if (skill.when.length === 0)
        return true;
...
export class PromptingSkillRegistry {
    ...
    forStep(category, goal, observation) {
 ...
 * any published one.
 */
export function builtInPromptingSkills() {
    return [
        parsePromptingSkill({
            name: "recall-what-i-know",
            category: "perception",
            title: "Recall what I already know",
            description: "Searches long-term memory for anything related to the goal before doing anything else, so the agent does not go looking outside for something it was already told.",
            author: "neuroclaw",
            source: "memory",
            query: "{goal}",
            priority: 100,
        ...
        parsePromptingSkill({
            ...
            description: "Searches earlier conversations for anything about this, so a question that follows on from something you said last week does not start from nothing.",
            author: "neuroclaw",
            source: "chats",
            query: "{goal}",
        ...
        parsePromptingSkill({
            ...
            description: "Decomposes the goal into subproblems using the reasoning engine, which is what turns a vague instruction into something the action step can actually attempt.",
            author: "neuroclaw",
            strategy: "decompose",
            priority: 100,
        }),
        parsePromptingSkill({
            ...
            description: "Hands the current step to the Tools plugin (calculation, hashing, encoding, dates). Fires only when the goal mentions something Tools genuinely covers, because an action skill that grabs every goal would starve the others.",
            author: "neuroclaw",
            plugin: "tools",
            input: "{goal}",
