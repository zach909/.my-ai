...
export class RemoteAccessStore {
    ...
    read() {
        ...
        try {
            ...
            if (typeof raw?.salt !== "string" || typeof raw?.hash !== "string")
                return null;
            return raw;
        }
        catch {
     ...
     */
    async set(password, authorised) {
        if (!authorised) {
            throw new RemoteAccessError(this.isSet()
    ...
    async check(password) {
        ...
        let supplied;
        try {
            supplied = await derive(password, Buffer.from(credential.salt, "base64"), credential.iterations || ITERATIONS);
        }
        catch {
            return false;
        }
...
export function getRemoteAccessStore() {
    if (!shared)
        shared = new RemoteAccessStore();
    return shared;
}
