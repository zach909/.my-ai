...
function neuronNameFor(requirement) {
    const cleaned = requirement
        ...
        .slice(0, 4)
        .join("-");
    return cleaned || `requirement-${Math.abs(hash(requirement)) % 10000}`;
}
function hash(text) {
    let h = 0;
 ...
 */
export function planRequirements(requirements, context = {}) {
    ...
    for (const plan of plans) {
        for (const neuron of plan.neurons) {
            if (seen.has(neuron.name))
                continue;
            seen.add(neuron.name);
            neurons.push(neuron);
        }
    }
    const rationale = needBuilding.length === 0
    ...
    return {
        ...
        alternative: {
            ...
            reason: wouldTrainBetter.length === 0
...
function toNeuroLang(neurons) {
    ...
    for (const neuron of neurons) {
        ...
        if (neuron.definition)
            lines.push(`"${neuron.name}"@definishon="${neuron.definition.replace(/"/g, "'")}"`);
    }
