/**
 * OneBrain — the NeuroClaw engine, in one file.
 *
 * Every background/foreground feature the engine is built from lives here:
 * quantization (dynamic/static, symmetric/asymmetric/mixed, real bit-packing),
 * the elastic zero-sum value budget, the all-to-all neuron mesh (and its
 * extension-builder-editable variant), the Mixture-of-Experts router,
 * hyperdimensional multi-ball neuron state, the compressed zip-loop I/O
 * system, and the quantum-interference neural net.
 *
 * This file is intentionally the single source of truth for all of it —
 * every one of the individual `<name>.ts` files this used to be split across
 * (quantizer.ts, quantization-config.ts, quantization-scheduler.ts,
 * quantization-hardware.ts, value-range.ts, mesh.ts, elastic-core.ts,
 * hyperdimensional.ts, quantum-net.ts, zip-io.ts, zip-io-loop.ts,
 * moe-router.ts, dual.ts, complex.ts) now just re-exports from here, so
...
export function detectHardwareProfile() {
    ...
    const simdWidthBits = arch64 ? (cores >= 4 ? 256 : 128) : 64;
    const supportsInt8Dot = arch64 && cores >= 2;
    const preferredBits = cores <= 1 ? 4 : supportsInt8Dot ? 8 : 8;
    const preferredKernel = supportsInt8Dot ? 'int8-dot' : simdWidthBits >= 128 ? 'simd128' : 'scalar';
    return {
...
function matchesRule(layerKey, rule) {
...
export function computeSensitivity(weights) {
    if (weights.length === 0)
        return 0;
    let sum = 0;
...
export function resolveLayerBits(layerKey, weights, policy) {
    for (const rule of policy.rules) {
        if (matchesRule(layerKey, rule))
            return clampToValidBits(rule.bits);
    }
    if (policy.sensitivityAdjust) {
        const sensitivity = computeSensitivity(weights);
        if (sensitivity > 1)
            return clampToValidBits(policy.defaultBits + 4);
        if (sensitivity < 0.1)
            return clampToValidBits(policy.defaultBits - 2);
    }
    return clampToValidBits(policy.defaultBits);
}
function clampToValidBits(bits) {
    return Math.max(2, Math.min(16, Math.floor(bits)));
}
/** Float32 tensor -> packed N-bit tensor memory delta. */
export function estimateMemorySavings(elementCount, bits) {
 ...
 */
export function estimatePowerSavingsPercent(bits, profile) {
 ...
 */
export function estimatePerformanceGain(bits, profile) {
...
export class HyperDimensionalEngine {
     ...
     */
    predictSelfModelWithDerivative(vec, velocity) {
        const dims = this.config.dimensions;
        const rank = this.config.selfModelRank;
        const h = Array.from({ length: rank }, () => dual(0, 0));
        for (let d = 0; d < dims; d++) {
        ...
        const value = new Array(dims).fill(0);
        const derivative = new Array(dims).fill(0);
        for (let r = 0; r < rank; r++) {
            for (let d = 0; d < dims; d++) {
