...
export const STORE_KINDS = ["net-skills", "prompting", "mods", "plugins", "binaries", "source", "files", "wiki"];
...
export function itemDir(kind, name) {
...
export function assertSafeName(name) {
    if (!/^[A-Za-z0-9][A-Za-z0-9._-]{0,63}$/.test(name) || name.includes("..")) {
        throw new StoreError(`"${name}" is not a valid name — letters, digits, '.', '-' and '_' only, up to 64 characters, and it cannot contain '..'.`);
    }
}
...
export function publishItem(input) {
    ...
    for (const f of existing?.files ?? [])
        carried.set(f.filename, { ...f, local: true });
...
export async function publishAndSync(input) {
    ...
    const sync = await syncStorePaths([itemDir(item.kind, item.name)], `store: publish ${item.kind}/${item.name}`, { storeDir: storeRoot() });
    return { item, sync };
}
/**
 ...
 * mean what it says.
 */
export async function deleteAndSync(kind, name) {
    assertKind(kind);
    assertSafeName(name);
...
export function readItem(kind, name) {
    assertKind(kind);
    assertSafeName(name);
    const dir = itemDir(kind, name);
    ...
    const indexed = Array.isArray(manifest.files) ? manifest.files : null;
...
export function readItemFile(kind, name, filename) {
    assertKind(kind);
    assertSafeName(name);
 ...
 */
export function deleteItem(kind, name) {
    assertKind(kind);
    assertSafeName(name);
    const dir = itemDir(kind, name);
    if (!existsSync(dir))
