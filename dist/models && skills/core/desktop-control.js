...
export class ToolMissing extends DesktopError {
    constructor(tool) {
        super(`"${tool}" is not installed. It is needed for ${DESKTOP_TOOLS[tool].purpose}. Install the "${DESKTOP_TOOLS[tool].package}" package.`);
        this.tool = tool;
    }
}
...
async function have(tool, fresh = false) {
    const cached = toolCache.get(tool);
    if (!fresh && cached && Date.now() - cached.at < TOOL_CACHE_MS)
...
export function forgetDetectedTools() {
    toolCache.clear();
}
/**
...
export class DesktopControl {
    ...
    async probe() {
        ...
        const names = Object.keys(DESKTOP_TOOLS);
        ...
        const found = await Promise.all(names.map(async (t) => [t, await have(t, true)]));
        const tools = Object.fromEntries(found);
    ...
    async requireTool(tool) {
        if (!(await have(tool)))
            throw new ToolMissing(tool);
    }
    gate(capability) {
    ...
    async listWindows() {
        ...
        if (!res.ok)
            throw new DesktopError(`Could not list windows: ${res.stderr.trim() || "wmctrl failed"}`);
     ...
     */
    async agentWindowIds() {
        if (!(await have("xdotool")))
            return new Set();
        const res = await run("xdotool", ["search", "--name", AGENT_WINDOW_MARKER]);
        if (!res.ok)
            return new Set();
        return new Set(res.stdout
            .split("\n")
            .map(s => s.trim())
            .filter(Boolean)
    ...
    async screenshot() {
        ...
        try {
            ...
            if (!res.ok)
                throw new DesktopError(`Screenshot failed: ${res.stderr.trim() || "gnome-screenshot failed"}`);
     ...
     */
    async launchAgentWindow(command, args = []) {
        ...
        child.unref();
        if (!child.pid)
            throw new DesktopError(`Could not launch "${command}".`);
    ...
    async requireAgentWindow(windowId) {
        const windows = await this.listWindows();
        const target = windows.find(w => w.id === windowId);
        if (!target)
            throw new DesktopError(`No window with id ${windowId}.`);
        if (!target.agentOwned) {
            throw new DesktopError(`Window ${windowId} ("${target.title}") belongs to the user. The agent can observe it but not control it.`);
        }
    }
    async moveWindow(windowId, x, y, width, height) {
        ...
        if (!res.ok)
            throw new DesktopError(`Could not move window: ${res.stderr.trim()}`);
    }
    async closeWindow(windowId) {
        ...
        if (!res.ok)
            throw new DesktopError(`Could not close window: ${res.stderr.trim()}`);
    }
    // ── input ───────────────────────────────────────────────────────────────
    /**
    ...
    async typeInto(windowId, text) {
        ...
        if (!focus.ok)
            throw new DesktopError(`Could not focus window ${windowId}: ${focus.stderr.trim()}`);
        // `--` so text starting with a dash is typed, not read as options.
        const res = await run("xdotool", ["type", "--delay", "12", "--", text]);
        if (!res.ok)
            throw new DesktopError(`Could not type: ${res.stderr.trim()}`);
    }
    async clickIn(windowId, x, y, button = 1) {
        ...
        if (!move.ok)
            throw new DesktopError(`Could not move the pointer: ${move.stderr.trim()}`);
        const click = await run("xdotool", ["click", String(button)]);
        if (!click.ok)
            throw new DesktopError(`Could not click: ${click.stderr.trim()}`);
    }
}
