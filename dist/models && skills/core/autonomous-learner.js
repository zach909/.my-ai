...
export class AutonomousLearner {
    ...
    findConflict(subject, relation, object) {
        const negOf = new Map();
...
function estimateReliability(text) {
    ...
    for (const h of HEDGE_WORDS)
        if (t.includes(h))
            score -= 0.2;
    for (const c of CONFIDENT_WORDS)
        if (t.includes(c))
            score += 0.15;
    if (/\d/.test(t))
        score += 0.05; // specific/measured claims skew slightly more reliable
...
function extractTriple(text) {
    ...
    if (relation === "isnt" || relation === "is not")
        relation = "is-not";
    if (relation === "cant")
        relation = "cannot";
