...
export class SelfModel {
     ...
     */
    calibrate(statedConfidence, domain) {
        const s = this.domains.get(normalize(domain));
        const c = clamp01(statedConfidence);
        if (!s || s.attempts < this.minAttempts)
            return clamp01(0.5 + (c - 0.5) * 0.5); // low evidence → shrink toward prior
