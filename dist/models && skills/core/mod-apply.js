...
import { assertSafeName, readItem, readItemFile } from "./store.js";
 ...
 */
export function modsTargetRoot() {
 ...
 * undo the second one.
 */
export async function applyMod(name) {
    const item = readItem(MOD_KIND, name);
    if (!item)
        throw new ModApplyError(`There is no published mod "${name}" to apply.`);
    if (item.files.length === 0)
        throw new ModApplyError(`"${name}" has no files to apply.`);
    ...
    if (changed.length === 0 && created.length === 0 && unchanged.length === 0) {
        throw new ModApplyError(`Could not apply "${name}": none of its files could be obtained. ${missing[0]?.reason ?? ""}`.trim());
    }
    const record = {
        name: item.name,
        title: item.title,
...
export function readApplied(name) {
...
export function listAppliedMods() {
    const root = backupRoot();
    if (!existsSync(root))
        return [];
    const out = [];
    for (const name of readdirSync(root)) {
        if (!statSync(path.join(root, name)).isDirectory())
            continue;
...
export function appliedChangedFiles(record, published) {
    const applied = new Map(record.files.map(f => [f.filename, f.modSha256]));
    const changed = [];
    for (const f of published.files) {
        if (applied.get(f.filename) !== f.sha256)
            changed.push(f.filename);
    }
    return changed.sort();
}
export function outdatedAppliedMods() {
    const out = [];
 ...
 */
export function revertMod(name) {
    const record = readApplied(name);
    if (!record)
        throw new ModApplyError(`"${name}" is not currently applied.`);
    const dir = backupDir(name);
    const restored = [];
    const removed = [];
    const skipped = [];
    ...
    if (skipped.length === 0) {
        ...
        const remaining = { ...record, files: record.files.filter(f => skipped.some(s => s.filename === f.filename)) };
