...
import { listCatalog, storeRoot, STORE_KINDS } from "./store.js";
const GIT_TIMEOUT_MS = 60000;
export class AutoUpdateError extends Error {
}
function git(args, cwd) {
    return new Promise(resolve => {
        execFile("git", args, { cwd, timeout: GIT_TIMEOUT_MS, maxBuffer: 8 * 1024 * 1024, env: { ...process.env, GIT_TERMINAL_PROMPT: "0" } }, (error, stdout, stderr) => {
...
export async function checkForUpdates() {
    ...
    const code = { behind: 0, ahead: 0, branch: "", commits: [] };
    ...
    if (code.behind > 0) {
        const log = await git(["log", "--oneline", `--max-count=${MAX_COMMITS_LISTED}`, `HEAD..origin/${code.branch}`], cwd);
        if (log.ok && log.out)
            code.commits = log.out.split("\n");
    }
    return { code, store: await staleDownloads(), checkedAt };
}
/**
...
export async function applyUpdates(opts = {}) {
    ...
    const cwd = repoDir();
    const result = { codeUpdated: false, filesRefreshed: [], fileProblems: [], restartRequired: false };
    if (doCode) {
        const branch = await git(["symbolic-ref", "--short", "HEAD"], cwd);
        if (!branch.ok || !branch.out) {
