...
import { listWikiPages, readWikiPage, publishWikiPageAndSync, deleteWikiPageAndSync, WikiNameError, } from "../models && skills/core/wiki-store.js";
...
export class WikiPlugin extends BasePlugin {
    ...
    async publish(name, title, content) {
        if (typeof name !== "string") {
            throw new Error("Security Error: Page name must be a string.");
        }
        if (!name || name.trim().length === 0) {
    ...
    async onMessage(message) {
        ...
        if (searchMatch?.[1]) {
            ...
            if (hits.length === 0)
                return `[Wiki] No pages matching "${searchMatch[1]}".`;
        ...
        if (readMatch?.[1]) {
            ...
            if (!page)
                return `[Wiki] No page named "${readMatch[1]}".`;
