...
export class ImageExtension extends BasePlugin {
    async onMessage(message) {
        ...
        if (input.startsWith('resize ') || input.startsWith('scale ')) {
            ...
            if (!isSafePath(path))
                return { error: 'Unsafe path pattern detected' };
        ...
        if (input.startsWith('convert ') || input.startsWith('format ')) {
            ...
            if (!isSafePath(src))
                return { error: 'Unsafe path pattern detected' };
            if (!isSafeFmt(fmt))
                return { error: 'Unsafe format pattern detected' };
        ...
        for (const cmd of [
            ...
            try {
                ...
                if (existsSync(outPath))
                    return { type: 'image', path: outPath, width: w, height: h, prompt };
            }
            catch {
                continue;
            }
    ...
    async resizeImage(path, w, h) {
        if (!isSafePath(path))
            return { type: 'image', error: 'Unsafe path pattern detected' };
        if (isNaN(w) || isNaN(h) || w <= 0 || h <= 0)
            return { type: 'image', error: 'invalid dimensions' };
        ...
        for (const cmd of [`convert "${path}" -resize ${w}x${h}! "${out}"`, `ffmpeg -i "${path}" -vf scale=${w}:${h} "${out}" -y 2>/dev/null`]) {
            try {
                execSync(cmd, { timeout: 10000 });
                return { type: 'image', path: out, width: w, height: h };
            }
            catch {
                continue;
            }
    ...
    async convertFormat(path, fmt) {
        if (!isSafePath(path))
            return { type: 'image', error: 'Unsafe path pattern detected' };
        if (!isSafeFmt(fmt))
            return { type: 'image', error: 'Unsafe format pattern detected' };
        const out = path.replace(/\.[^.]+$/, `.${fmt}`);
        for (const cmd of [`convert "${path}" "${out}"`, `ffmpeg -i "${path}" "${out}" -y 2>/dev/null`]) {
            try {
                execSync(cmd, { timeout: 10000 });
                return { type: 'image', path: out, format: fmt };
            }
            catch {
                continue;
            }
    ...
    async imageInfo(path) {
        ...
        try {
            ...
            return { type: 'image', path, info: 'unreadable' };
        }
    }
...
export class VideoExtension extends BasePlugin {
    async onMessage(message) {
        ...
        if (input.startsWith('trim ')) {
            ...
            if (!isSafePath(path))
                return { error: 'Unsafe path pattern detected' };
            if (!isSafeTime(start) || !isSafeTime(duration))
                return { error: 'invalid timestamp/duration' };
    ...
    async generateVideo(desc) {
        ...
        try {
            ...
            if (existsSync(outPath))
                return { type: 'video', path: outPath, description: desc };
        }
        catch { /* fall through */ }
        try {
    ...
    async trimVideo(path, start, duration) {
        if (!isSafePath(path))
            return { type: 'video', error: 'Unsafe path pattern detected' };
        if (!isSafeTime(start) || !isSafeTime(duration))
            return { type: 'video', error: 'invalid timestamp/duration' };
        ...
        try {
            ...
            return { type: 'video', error: 'Trim failed', path };
        }
    }
    async concatVideos(paths) {
        if (paths.length < 2)
            return { type: 'video', error: 'Need at least 2 files' };
        for (const pt of paths) {
            if (!isSafePath(pt))
                return { type: 'video', error: 'Unsafe path pattern detected' };
        }
        ...
        try {
            ...
            return { type: 'video', error: 'Concat failed' };
        }
    }
    async extractAudio(path) {
        if (!isSafePath(path))
            return { type: 'video', error: 'Unsafe path pattern detected' };
        ...
        try {
            ...
            return { type: 'video', error: 'Extract audio failed', path };
        }
    }
    async videoInfo(path) {
        ...
        try {
            ...
            return { type: 'video', path, info: 'unreadable' };
        }
    }
    async onHealthCheck() { return true; }
}
    ...
    doAction(action) {
        for (const [id, g] of this.games) {
            ...
            if (g.type === 'guess') {
                ...
                if (num === gs.target) {
                    gs.state = 'won';
                    return { type: 'game', id, result: 'won', turns: gs.turns, message: `Correct! ${gs.target} in ${gs.turns} turns` };
                }
            ...
            if (g.type === 'maze') {
                ...
                if (nx < 0 || nx >= gm.grid || ny < 0 || ny >= gm.grid)
                    return { type: 'game', id, error: 'wall', pos: gm.pos };
                gm.pos = { x: nx, y: ny };
                if (nx === gm.exit.x && ny === gm.exit.y) {
                    gm.state = 'won';
                    return { type: 'game', id, result: 'won', turns: gm.turns, pos: gm.pos, message: 'Escaped the maze!' };
                }
...
export class PluginMakerExtension extends BasePlugin {
    ...
    generateManifest(name, desc, caps) {
