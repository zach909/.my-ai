 ...
 */
import { installPromptingSkill, listInstalled, loadRegistry, publishPromptingSkill, } from "../models && skills/core/prompting-skill-store.js";
import { BasePlugin } from "../plugin_manager/sdk.js";
import { STORE_KINDS, STORE_KIND_LABELS, listCatalog, publishAndSync, readItem, readItemFile, } from "../models && skills/core/store.js";
import { installItem, isInstalled, listInstalledItems, outdatedInstalls, uninstallItem, updateInstalls, } from "../models && skills/core/store-install.js";
export class StorePlugin extends BasePlugin {
     ...
     */
    async readFile(kind, name, filename) {
        const local = readItemFile(kind, name, filename);
        if (local)
            return local.toString("utf8");
        try {
     ...
     */
    async editFile(kind, name, filename, content) {
     ...
     */
    addFile(kind, name, filename, content, encoding = "utf8") {
     ...
     */
    async describe(kind, name, fields) {
        // async for the same reason as editFile above.
        const existing = readItem(kind, name);
        if (!existing)
            throw new Error(`No "${name}" in ${kind} to describe.`);
    ...
    async onMessage(message) {
        ...
        if (show) {
            ...
            if (!item)
                return { tool: "store", result: `No "${show[2]}" in ${show[1]}.` };
            const here = isInstalled(kind, item.name);
            return {
                tool: "store",
                result: `${item.title} (${item.kind}/${item.name}) by ${item.author}\n` +
        ...
        if (publishHead) {
            ...
            try {
                ...
                return {
                    tool: "store",
                    result: `Published ${item.kind}/${item.name} — ${filename} (${content.length} bytes).\n` +
        ...
        if (/^store\s+publish\b/i.test(input) && !publishHead) {
            return {
                tool: "store",
                result: 'To publish, put the file on the lines after the command:\n' +
                    'store publish <kind> <name> <filename>\n<the file contents>\n\n' +
                    `Kinds: ${STORE_KINDS.join(", ")}.`,
            };
        }
        ...
        if (/^store\s+installed$/i.test(input)) {
            ...
            if (items.length === 0)
                return { tool: "store", result: "Nothing from the store is installed on this device." };
            return {
                tool: "store",
                result: items
        ...
        if (/^store\s+(outdated|updates?)$/i.test(input)) {
            ...
            if (out.length === 0)
                return { tool: "store", result: "Everything installed is up to date." };
            return {
                tool: "store",
