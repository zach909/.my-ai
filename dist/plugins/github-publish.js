...
import { publishAndSync, itemDir, StoreError } from "../models && skills/core/store.js";
...
function looksLikeWeights(filename) {
...
export class GithubPublishPlugin extends BasePlugin {
    ...
    async push(input) {
        ...
        if (weighty.length > 0) {
            throw new StoreError(`Refusing to publish ${weighty.map(f => f.filename).join(", ")}: this looks like model weights, ` +
    ...
    async onMessage(message) {
        ...
        if (/^(?:github\s+push|push\s+to\s+github|publish\s+to\s+github)\b/i.test(input) && !pushCmd) {
            return {
                tool: "github-publish",
                result: "To push, put the file on the lines after the command:\n" +
