...
export class DesktopPlugin extends ToolPlugin {
     ...
     *                 edits.
     */
    constructor(definition, control) {
        super(definition);
    ...
    desktop() {
        if (!this.control)
            this.control = new DesktopControl(sharedAccessManager());
        return this.control;
    }
    defineTools() {
        this.defineTool({
        ...
        this.defineTool({
            ...
            run: async (args) => this.desktop().launchAgentWindow(textArg(args, "command"), Array.isArray(args.args) ? args.args.map(String) : []),
        });
        this.defineTool({
            name: "move_window",
            ...
            run: async (args) => {
                await this.desktop().moveWindow(textArg(args, "windowId"), numberArg(args, "x"), numberArg(args, "y"), numberArg(args, "width"), numberArg(args, "height"));
        ...
        this.defineTool({
            ...
            run: async (args) => {
                await this.desktop().clickIn(textArg(args, "windowId"), numberArg(args, "x"), numberArg(args, "y"), args.button === undefined ? 1 : numberArg(args, "button"));
