...
import { ACCESS_SWITCHES, SWITCH_LABEL, } from "../models && skills/core/access-manager.js";
...
export class ComputerAccessPlugin extends BasePlugin {
    ...
    workspace() {
        if (!this.workspaceInstance)
            this.workspaceInstance = new AgentWorkspace(sharedAccessManager(), process.cwd());
        return this.workspaceInstance;
    }
    /** The GNOME graphical half. */
    desktop() {
        if (!this.desktopInstance)
            this.desktopInstance = new DesktopControl(sharedAccessManager());
     ...
     */
    turnOff(name) {
        if (!ACCESS_SWITCHES.includes(name)) {
    ...
    async onMessage(message) {
        ...
        // want it to stop.
        const off = input.match(/^(?:turn off|disable|stop|kill)\s+(?:the\s+)?(all|everything|computer|full|desktop|gnome|screen|terminal|files?|workspace)\s*(?:access)?$/i);
        if (off) {
            const word = off[1].toLowerCase();
            const name = /desktop|gnome|screen/.test(word)
                ? "desktop"
        ...
        if (/^(?:turn on|enable|allow|restore)\s+(?:the\s+)?\w+\s*(?:access)?$/i.test(input)) {
            ...
            return {
                tool: "computer-access",
                result: "I can turn computer access off, but not back on — that has to be done by a person, on the Access page in the interface.",
            };
        }
        return null;
    }
