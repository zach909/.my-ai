...
export class CLI {
    ...
    async drainQueue() {
        ...
        while (this.commandQueue.length > 0) {
            ...
            try {
                ...
                console.error(this.colorize(RED, `  Error: ${e}`));
            }
        }
        this.queueRunning = false;
        this.rl?.prompt();
    ...
    async startInteractive() {
        ...
        this.rl.on('line', (line) => {
            ...
            if (lower.startsWith('generate ') || trimmed.startsWith('"') || trimmed.startsWith("'")) {
                ...
                this.enqueue(() => this.handleGenerate(prompt));
                return;
            }
    ...
    async handleThink(prompt) {
        if (!prompt) {
            console.log(this.colorize(GRAY, '  Usage: think <prompt>'));
            return;
        }
        const before = Date.now();
    ...
    async handleTrain(text) {
        ...
        // from only this one string, erasing every earlier `train` call in
        // the same session. See NeuroclawTrainer.learnText() and
    ...
    handleSearch(query) {
        if (!query) {
            console.log(this.colorize(GRAY, '  Usage: search <query>'));
            return;
        }
    ...
    async handleNeuri(code) {
        ...
        for (const [name, n] of neurons) {
            ...
            const tags = [];
            if (n.isNetSearch)
                tags.push(`netsearch:${n.netLocation}`);
            if (n.isCodeNet)
                tags.push('code-net');
            if (n.code)
                tags.push(`code:"${n.code.slice(0, 30)}"`);
    ...
    handleBuild(name) {
        if (!name) {
            console.log(this.colorize(GRAY, '  Usage: build <extension-name>'));
            return;
        }
    ...
    printPlugins() {
        ...
        if (plugins.length > 15)
            console.log(this.colorize(GRAY, `    ... and ${plugins.length - 15} more`));
        console.log('');
