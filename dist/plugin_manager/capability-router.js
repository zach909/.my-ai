 ...
 */
export function describesCapabilities(p) {
    return typeof p?.describeCapabilities === "function";
}
...
export class RoutingMemory {
    ...
    recordSuccess(input, pluginId) {
        ...
        for (const term of terms) {
            const forTerm = ((_a = this.evidence)[term] ?? (_a[term] = {}));
        ...
        if (this.records % 25 === 0)
            this.decay();
        if (Object.keys(this.evidence).length > MAX_LEARNED_TERMS)
            this.prune();
    }
    decay() {
        for (const [term, plugins] of Object.entries(this.evidence)) {
            for (const [id, weight] of Object.entries(plugins)) {
                ...
                if (next < MIN_EVIDENCE)
                    delete plugins[id];
                else
                    plugins[id] = next;
            }
            if (Object.keys(plugins).length === 0)
                delete this.evidence[term];
        }
    }
    /** Drop the weakest terms when the table grows past its bound. */
    prune() {
        const strength = (plugins) => Math.max(...Object.values(plugins));
    ...
    boostFor(pluginId, terms) {
        let total = 0;
        for (const term of terms)
            total += this.evidence[term]?.[pluginId] ?? 0;
    ...
    import(data) {
        if (!data || typeof data.evidence !== "object" || !data.evidence)
            return;
...
export class CapabilityRouter {
    ...
    reindex(plugins, fallbackCapabilities = {}) {
        this.index = [];
        for (const [id, plugin] of plugins) {
            const declared = describesCapabilities(plugin)
            ...
            const terms = new Set();
            for (const term of tokenize(id.replace(/-/g, " ")))
                terms.add(term);
            for (const cap of fallbackCapabilities[id] ?? []) {
                for (const term of tokenize(cap.replace(/-/g, " ")))
                    terms.add(term);
            }
            ...
            for (const verb of declared.verbs ?? [])
                terms.add(stem(verb.toLowerCase()));
            for (const noun of declared.nouns ?? [])
                terms.add(stem(noun.toLowerCase()));
            this.index.push({
                id,
    ...
    rank(input, intentCandidates = []) {
        ...
        for (const entry of this.index) {
            ...
            if (overlap > 0) {
                score += overlap * 4;
                if (!reason)
                    reason = `${overlap} matching term${overlap === 1 ? "" : "s"}`;
            }
            ...
            if (learned > 0) {
                score += learned;
                if (!reason)
                    reason = "handled similar requests before";
            }
            score += entry.weight;
            if (score > 0) {
                ranked.push({
