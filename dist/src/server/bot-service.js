 ...
 */
import { logError, getRecentErrors } from '../lib/error-log.js';
...
const NEGATIVE_WORDS = /\b(sad|lost|lonely|angry|upset|hate|terrible|awful|worried|anxious|scared|hurt|broke|broken|failed|failure|depressed|alone|tired|stressed|frustrated|fired|laid off|rejected|devastated|heartbroken|grief|grieving)\b/i;
const POSITIVE_WORDS = /\b(great|good|love|happy|excited|awesome|thanks|thank you|wonderful|glad|win|won|success|proud|relieved|grateful)\b/i;
 ...
 */
export function empathyPrefix(userMessage) {
...
export function detectAmbiguousIntent(message) {
    if (PLAN_SIGNAL.test(message) && RECALL_SIGNAL.test(message)) {
        return {
            message: "Before I act on this, I want to make sure I'm not guessing: this reads like it could be asking me to plan something new, or to recall something from before — and those are different enough that I'd rather ask than assume. Which did you mean?",
            confidence: 0.5,
...
export function applyHumility(message, confidence) {
    const cappedConfidence = Math.min(confidence, MAX_HONEST_CONFIDENCE);
    if (cappedConfidence >= HEDGE_THRESHOLD)
        return { message, confidence: cappedConfidence };
    return {
...
export class ChatBot {
    constructor() {
        ...
        this.contextWindow = 10; // Keep last N messages for context
    }
    ...
    async processMessage(userMessage) {
        ...
        this.trimHistory();
        try {
            const raw = detectAmbiguousIntent(userMessage) ??
            ...
            const alreadyHandled = raw.metadata?.domain === 'route' || raw.metadata?.domain === 'error' || raw.metadata?.domain === 'clarify';
     ...
     */
    applyBehavior(userMessage, response) {
    ...
    async processWithSystem(userMessage) {
        if (!this.system) {
     ...
     */
    async tryAgentLoop(userMessage) {
        if (!this.system)
            return null;
        try {
            ...
            if (!run || !run.answered)
                return null;
            const skillsUsed = [...new Set(run.result.steps.map(s => s.skill).filter(Boolean))];
            return {
                message: run.message,
                confidence: 0.9,
     ...
     */
    async processSimplified(userMessage) {
    ...
    isRouteQuery(lowerMessage) {
        return (/\b(where|which page|which route|navigate|go to|take me to)\b/.test(lowerMessage) &&
            /\b(page|pages|route|routes|section|screen|tab)\b/.test(lowerMessage)) || /\blist (all )?(the )?(pages|routes)\b/.test(lowerMessage);
    }
    isErrorQuery(lowerMessage) {
    ...
    generateResponse(userMessage) {
        ...
        if (lower.includes('hello') || lower.includes('hi')) {
            return {
                message: "Hello! I'm here to help you with analysis, planning, coding, problem-solving, and creative thinking. What would you like to work on?",
                confidence: 0.95,
                domain: 'greeting',
            };
        }
     ...
     */
    buildAnalyticalResponse(userMessage, analysis) {
        const pools = {
            coding: [
     ...
     */
    generateSuggestions(userMessage, responseMessage, domain) {
        const byDomain = {
            coding: [
...
export async function getBot(system) {
    if (!botInstance) {
