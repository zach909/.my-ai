 ...
 */
function tryExactArithmetic(text) {
...
export class NeuroclawSystem {
    ...
    async recordCorrection(input) {
        ...
        if (!corrected)
            return { applied: false, forgot: 0, reason: "the correction is empty" };
        if (original === corrected)
            return { applied: false, forgot: 0, reason: "nothing changed" };
        ...
        for (const item of this.memory.all()) {
            if (item.content === stored && this.memory.forget(item.id))
                forgot++;
        }
    ...
    async processQueryImpl(input) {
        ...
        if (exactMath)
            return this.respondDirect(exactMath, turnImportance);
        ...
        if (result.decision === "recommend-skill" || result.decision === "recommend-extension") {
            ...
            if (created) {
                try {
                    ...
                    let parsed = {};
                    try {
                        const candidate = JSON.parse(created);
    ...
    async askHiveAgentImpl(role, task) {
        ...
        if (!decision.allowed)
            return { error: `[Withheld] ${decision.reasons.join("; ")}` };
     ...
     */
    async summonHiveAgent(role, specialization, task) {
        if (!this.initialized)
            await this.initialize();
        return this.trackCall("summon-hive-agent", async () => this.summonHiveAgentImpl(role, specialization, task));
    }
    async summonHiveAgentImpl(role, specialization, task) {
        this.ensureDefaultTeam();
    ...
    async solveImpl(problem, opts) {
        ...
        if (unsupportedAssumptions.length > 0)
     ...
     */
    async compareAnswers(candidates) {
