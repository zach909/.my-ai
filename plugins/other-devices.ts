...
export class OtherDevicesPlugin extends BasePlugin {
  ...
  async disconnect(id: string): Promise<boolean> {
    ...
    if (registered) { registered.connected = false; return true; }
  ...
  async reconnect(id: string): Promise<boolean> {
    this.validateString(id, "Device ID");
    const registered = this.devices.find(dev => dev.id === id);
    if (registered) { registered.connected = true; registered.lastSeen = Date.now(); return true; }
    if (this.disconnected.has(id)) { this.disconnected.delete(id); return true; }
    return false;
  }
