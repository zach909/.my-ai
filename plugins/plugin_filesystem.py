...
class FileSystemPlugin(Plugin):
    ...
    def _resolve(self, path: str) -> str:
        ...
        # Use realpath to resolve symlinks and ensure we stay in root
    ...
    def _sanitize_pattern(self, pattern: str) -> str:
        # Cross-platform check for absolute patterns and directory traversal
        if ".." in pattern or os.path.isabs(pattern) or pattern.startswith("~"):
    ...
    def _list(self, path: str = ".", pattern: str = "*") -> List[str]:
        path = self._resolve(path)
        pattern = self._sanitize_pattern(pattern)
