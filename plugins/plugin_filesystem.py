...
class FileSystemPlugin(Plugin):
    ...
    def _resolve(self, path: str) -> str:
        # Security: Jail paths within the current working directory
        full_path = os.path.abspath(os.path.join(self._root, os.path.expanduser(path)))
        rel = os.path.relpath(full_path, self._root)
        if rel.startswith("..") or os.path.isabs(rel):
            raise ValueError(f"Security Error: Path traversal detected: {path}")
        return full_path

    def _sanitize_pattern(self, pattern: str) -> None:
        # Security: Block traversal characters in glob patterns
        if ".." in pattern or pattern.startswith("/") or "~" in pattern:
            raise ValueError(f"Security Error: Dangerous pattern detected: {pattern}")
    ...
    def _list(self, path: str = ".", pattern: str = "*") -> List[str]:
        path = self._resolve(path)
        self._sanitize_pattern(pattern)
