...
export class MultiDesktopManager {
    ...
    createAiVirtualPointer() {
        ...
        if (this.uinputAvailable)
            return this.createUinputDevice('pointer');
    ...
    createAiVirtualKeyboard() {
        ...
        if (this.uinputAvailable)
            return this.createUinputDevice('keyboard');
        return this.createSimulatedDevice('keyboard');
    }
    createXinputPointer(name) {
        try {
    ...
    bindDeviceToWorkspace(deviceId, workspace) {
    ...
    createDesktop(name, isAi = false) {
        if (this.gnomeAvailable) {
            const count = this.getDesktopCount();
            const newId = count;
            execSync(`gsettings set org.gnome.desktop.wm.preferences num-workspaces ${count + 1}`, { timeout: 5000 });
            const info = { id: newId, name: name ?? `Desktop ${newId + 1}`, active: false, isAiDesktop: isAi };
            if (isAi)
                this.aiWorkspace = newId;
            return info;
        }
        const id = this.simulatedDesktops.size;
        const info = { id, name: name ?? `Desktop ${id + 1}`, active: false, isAiDesktop: isAi };
    ...
    switchToDesktop(id) {
        if (this.gnomeAvailable) {
            try {
                execSync(`gdbus call --session --dest org.gnome.Shell --object-path /org/gnome/Shell --method org.gnome.Shell.Eval "global.workspace_manager.get_workspace_by_index(${id}).activate(global.get_current_time())" 2>/dev/null`, { timeout: 5000, encoding: 'utf8' });
            }
            catch {
                try {
                    execSync(`wmctrl -s ${id} 2>/dev/null`, { timeout: 3000 });
                }
                catch { }
            }
            return;
        }
        for (const [, d] of this.simulatedDesktops)
            d.active = d.id === id;
    }
    listDesktops() {
        if (this.gnomeAvailable) {
    ...
    getCurrentDesktop() {
        if (this.gnomeAvailable) {
            try {
                const r = execSync('gdbus call --session --dest org.gnome.Shell --object-path /org/gnome/Shell --method org.gnome.Shell.Eval "global.workspace_manager.get_active_workspace().index()" 2>/dev/null', { encoding: 'utf8', timeout: 5000 });
                const m = r.match(/\d+/);
                if (m)
    ...
    removeDesktop(id) {
        ...
        if (this.gnomeAvailable) {
            const count = this.getDesktopCount();
            if (count > 1) {
                if (this.getCurrentDesktop() >= id)
                    this.switchToDesktop(0);
                execSync(`gsettings set org.gnome.desktop.wm.preferences num-workspaces ${count - 1}`, { timeout: 5000 });
            }
            return;
        }
    ...
    launchOnDesktop(command, desktopId) {
    ...
    isolateAiInput() {
        ...
        try {
            ...
            for (const dev of this.virtualDevices.values()) {
                if (dev.active && dev.type === 'pointer') {
                    aiMasterId = dev.masterId;
                    break;
                }
            }
            if (aiMasterId < 0)
            ...
            for (const line of lines) {
                ...
                if (name.includes('XTEST') || name.includes('Virtual core'))
                    continue;
    ...
    removeVirtualDevicesForWorkspace(workspace) {
