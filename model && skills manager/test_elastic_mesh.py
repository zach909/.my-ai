...
2. The **elastic-mesh expert core** (`tinygpt/elastic_mesh.py`, Section 5.2) —
   the `ElasticMeshFFN` MoE block whose per-expert interference step is a
   genuine PennyLane statevector simulation of a small variational quantum
   circuit — still forward/backward-passes as a standalone drop-in module,
   preserving the (B, T, C) hidden shape it drops into.

The elastic-mesh core is an OPTIONAL component — it is not part of the
canonical all-to-all mesh that `build_model()` constructs, and it requires
PennyLane (`pip install pennylane`). If PennyLane isn't installed those
checks skip cleanly rather than failing, so `python test_core.py` stays
dependency-light.
...
# tinygpt.elastic_mesh (PennyLane-dependent) is imported defensively inside
# main() instead of here, so this module still loads — and the rest of the
# suite can still run — when PennyLane isn't installed.
...
def main():
    # PennyLane availability only gates the OPTIONAL Section 5.2 quantum
    # block (checks 0 and 4 below) — it must never skip checks 1-3, which
    # guard the canonical all-to-all mesh that build_model() actually returns
    # and have no PennyLane dependency at all.
    try:
        from tinygpt.elastic_mesh import ElasticMeshFFN
        elastic_mesh_available = True
    except Exception as e:                       # pennylane (or a dep) missing
        print(f"skip elastic-mesh (Section 5.2) checks — optional component unavailable: {e}")
        print("  (install with: pip install pennylane)")
        elastic_mesh_available = False

    from tinygpt.config import ModelConfig

    torch.manual_seed(0)

    # 0) the standalone Section 5.2 quantum block, in isolation
    if elastic_mesh_available:
