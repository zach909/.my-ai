...
class TSR(BaseModule):
    ...
    def from_pretrained(
        ...
        # Remap old checkpoint keys (transformers 4.x DINO) -> new keys (transformers 5.x)
        # Old: encoder.layer.0.attention.attention.query -> New: layers.0.attention.q_proj
        for k in list(ckpt.keys()):
            if 'image_tokenizer.model' not in k:
                continue
            new_k = k
            ...
            if new_k != k:
                ckpt[new_k] = ckpt.pop(k)

        try:
            model.load_state_dict(ckpt, strict=True)
        except RuntimeError as e:
            print(f"[tsr] checkpoint load warning (non-fatal): {e}", file=sys.stderr)
