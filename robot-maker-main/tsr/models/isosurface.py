...
def marching_cubes(level: torch.Tensor, level_set: float = 0.0):
    level_np = level.detach().cpu().numpy()
    verts, faces, _, _ = _sk_marching_cubes(level_np, level_set)
    v_pos = torch.from_numpy(verts.astype(np.float32))
    t_pos_idx = torch.from_numpy(faces.astype(np.int64).astype(np.int32))
