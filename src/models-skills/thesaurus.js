...
export class ThesaurusDictionary {
    ...
    async loadFromFile(filePath) {
        try {
            ...
            this.data.entries = new Map(Object.entries(parsed.entries).map(([k, v]) => [k, v]));
     ...
     */
    importData(data) {
        this.data.entries = new Map(Object.entries(data.entries).map(([k, v]) => [k, v]));
...
export class ThesaurusFactory {
    ...
    static createBasic() {
        ...
        for (const entry of entries)
            thesaurus.addWord(entry);
        return thesaurus;
    }
    /**
     * Create a technical thesaurus for coding
     */
    static createTechnical() {
        const thesaurus = new ThesaurusDictionary();
        const entries = [
