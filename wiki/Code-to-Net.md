...
|---|---|---|
| TypeScript runtime backend (structural) | `extension-builder/builder.ts` — `importCodeToNet(projectId, name, binaryCode)` → `core/thorns.ts` `CodeToNet` | Imports binary code into a new `codenet`-type neuron chain (topology) |
