...
## 2026-08-14 - Inconsistent Path Sandboxing and Glob Pattern Bypass in Python FileSystemPlugin
**Vulnerability:** While the TypeScript FileSystemPlugin was sandboxed, its Python counterpart (`plugins/plugin_filesystem.py`) lacked any path validation, allowing arbitrary read/write/delete access to the host system. Furthermore, even with path resolution hardening, `glob.glob` remained a vector for escaping the sandbox via ".." or absolute paths in the pattern argument.
**Learning:** Security parity must be maintained across all implementations of a feature. Hardening only the "main" (TS) entry point is insufficient if parallel tools (Python) exist. Additionally, when using wildcard-capable functions like `glob`, the pattern itself is a user-controlled input that requires its own validation separate from the base directory resolution.
**Prevention:** Always implement a central `_resolve` helper that uses `os.path.relpath` to jail paths. Explicitly block ".." and absolute prefixes in glob patterns to prevent pattern-based sandbox escapes.
