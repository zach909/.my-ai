...
## 2026-08-13 - MoERouter Shannon Entropy of Softmax Optimization
**Learning:** Standard Shannon entropy calculations ($H = -\sum p_i \ln p_i$) on a softmax probability vector require first computing and allocating an intermediate probability vector, followed by $O(N)$ expensive transcendental `Math.log` operations. Reformulating the Shannon entropy algebraically allows calculating the exact entropy directly from the raw pre-softmax scores in a single pass: $H = \ln\left(\sum e^{s_i - M}\right) - \frac{\sum (s_i - M) e^{s_i - M}}{\sum e^{s_i - M}}$. This completely avoids allocating any intermediate arrays (reducing GC pressure) and reduces the transcendental `Math.log` function calls from $O(E)$ to exactly $O(1)$.
**Action:** When computing Shannon entropy of a softmax distribution, bypass the explicit softmax vector generation and evaluate the exact simplified formulation directly over raw scores to achieve massive CPU and memory efficiency gains.
