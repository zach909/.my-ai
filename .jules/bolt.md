...
## 2026-07-25 - ESM Sibling Resolution Performance Trap
**Learning:** In codebases with mixed TypeScript and JavaScript sources, keeping stale duplicate hand-copied `.js` files in the source tree alongside their active `.ts` siblings creates a silent performance trap. When benchmarks or files import `./foo.js` (required by standard Node.js ESM rules for TypeScript compilation), runtimes like Bun will load the slow, stale `.js` file from disk instead of dynamically compiling and running the optimized `.ts` sibling, silently neutralizing massive algorithmic and cache speedups.
**Action:** Avoid keeping duplicate stale `.js` files next to `.ts` source files, and ensure compiled assets are fully rebuilt and synchronized to prevent massive performance regressions and drift.
